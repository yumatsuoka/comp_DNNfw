...
class Trainer:
    def __init__(self, model, train_data, val_data, batchsize=100):
        self.model = model
...
class Multigpu_trainer(Trainer):
    def __init__(self, model, train_data, val_data, batchsize=100, lr=1e-3, num_gpu=1):
        Trainer.__init__(self, model, train_data, val_data, batchsize)
        self.batchsize = batchsize/num_gpu
        ...
        for i in six.moves.range(self.num_gpu):
            with tf.device('/gpu:%d' %i):
                with tf.namescope('Tower_%d' %i) as scope:
                    ...
                    loss = tf.reduce_mean(tf.softmax_cross_entropy_with_logits(\
