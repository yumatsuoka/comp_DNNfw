...
class VGG16:
    ...
    def train(self):
        ...
        if tf.train.get_checkpoint_state('naive_model/'):
            saver = tf.train.Saver()
            ckpt = tf.train.get_checkpoint_state('naive_model/')
        ...
        while True:
            ...
            for i in tqdm(range(batch_index)):
                ...
                optimizer.run(feed_dict={self.x: x_batch.astype(np.float32), self.t: t_batch.astype(np.float32)}, session=sess)
                loss_value = self.loss.eval(feed_dict={self.x: x_batch.astype(np.float32), self.t: t_batch.astype(np.float32)}, session=sess)
                sum_loss += loss_value
            ...
            saver = tf.train.Saver()
            saver.save(sess, 'naive_model/naive_model', write_meta_graph=False)
            '''
