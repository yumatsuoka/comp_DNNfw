...
# Convolution2Dにおけるtensorのshapeについて注意点
#backgroundが'th'(theano)の場合は(samples, channels, rows, cols)
#'tf'の場合は(samples, rows, cols, channels)
# BatchNormalizationについても、backgroundで挙動が変わることに注意
...
def allconvnet(img_channels=3, img_rows=32, img_cols=32, nb_classes=10):
    ...
    ## stridced conv
    conv3 = Convolution2D(96, 3, 3, border_mode='valid', subsample=(2, 2))(relu2)
    ## add bn1
    bn1 = BatchNormalization()(conv3)
    relu3 = Activation('relu')(bn1)
    conv4 = Convolution2D(192, 3, 3, border_mode='same')(relu3)
    relu4 = Activation('relu')(conv4)
    conv5 = Convolution2D(192, 3, 3, border_mode='same')(relu4)
    relu5 = Activation('relu')(conv5)
    ## stridced conv
    conv6 = Convolution2D(192, 3, 3, border_mode='valid', subsample=(2, 2))(relu5)
    ## add bn2
    bn2 = BatchNormalization()(conv6)
    relu6 = Activation('relu')(bn2)
    conv7 = Convolution2D(192, 3, 3, border_mode='same')(relu6)
    relu7 = Activation('relu')(conv7)
    ## replace fc to 1x1 convolution
    conv8 = Convolution2D(192, 1, 1, border_mode='valid')(relu7)
